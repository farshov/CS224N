...
def pad_sents_char(sents, char_pad_token):
    ...
    max_sent_lenght = max(lengths)
    for s in sents:
        cur_sent = []
        for w in s:
            if len(w) >= max_word_length:
                cur_sent.append(w[0:max_word_length])
            else:
                cur_sent.append(w + [char_pad_token] * (max_word_length - len(w)))
        if len(s) < max_sent_lenght:
            cur_sent += [[char_pad_token] * max_word_length] * (max_sent_lenght - len(s))
        sents_padded.append(cur_sent)
    ### END YOUR CODE

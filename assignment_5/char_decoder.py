...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        input_embed = self.decoderCharEmb(input)  # Shape:  (length, batch, char_embedding_size)
        h_s, (hn, cn) = self.charDecoder(input_embed, dec_hidden)
        dec_hidden = (hn, cn)
        scores = self.char_output_projection(h_s)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        @param char_sequence: tensor of integers, shape (length, batch). Note that "length" here and in forward()
        ...
        scores, dec_hidden = self.forward(input, dec_hidden)  # Shape: length, batch, self.vocab_size
        ...
        return critrerion(scores, target)
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        decoding = [["", False] for _ in range(batch)]

        prev_hidden = initialStates
        input = torch.tensor([[self.target_vocab.start_of_word for i in range(batch)]], device=device)
        # Shape: (1, batch, hidden_size)
        for t in range(max_length):
            scores, prev_hidden = self.forward(input, prev_hidden)  # Shape: (1, batch, self.vocab_size)
            input = scores.argmax(dim=2)  # Shape: (1, batch)

            for i in range(input.shape[1]):
                if not decoding[i][1]:
                    symbol = self.target_vocab.id2char[input[0, i].item()]
                    if input[0, i].item() != self.target_vocab.end_of_word:
                        decoding[i][0] += symbol
                    else:
                        decoding[i][1] = True

        decodedWords = [word[0] for word in decoding]
